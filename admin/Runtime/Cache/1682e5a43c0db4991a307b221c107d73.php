...
<div class="col-sm-6 pull-right">
    <button class="btn btn-sm btn-primary pull-right" onclick="javascript:window.location.href = '?m=<?php echo ($mod); ?>'">
        返回列表
        ...
        <div class="widget-main">
            <form action="" method="post" name="myform" class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="rolename" value="<?php echo ($data["rolename"]); ?>" class="col-xs-10 col-sm-5">
                        <span class="help-inline col-xs-12 col-sm-7">
                            <span class="middle">*请输入角色名称</span>
                        </span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="description"  value="<?php echo ($data["description"]); ?>"  class="col-xs-10 col-sm-5">
                        <span class="help-inline col-xs-12 col-sm-7">
                            <span class="middle">*请输入角色描述</span>
                        </span>
                    </div>
                </div>
                ...
                <div class="form-group ">
                   <label class="col-sm-3 control-label padding-right"> 权限选择 </label>
                    <div class="col-sm-9">
                        <div class="checkbox">
                            <label>
                                <input name="checkall" type="checkbox" class="ace" id="cl">
                                <span class="lbl">全选/取消</span>
                            </label>
                        </div>
                        <?php if(is_array($node_arr)): foreach($node_arr as $key=>$v): ?><div class="checkbox ">
                                <label >
                                    <input name="nodeids[]" value="<?php echo ($v["id"]); ?>" type="checkbox"  <?php if(in_array(($v['id']), is_array($data['nodes'])?$data['nodes']:explode(',',$data['nodes']))): ?>checked<?php endif; ?>  class="ace" onclick="checknode(this)">
                                    <span class="lbl red"> <?php echo ($v["name"]); ?></span>
                                </label>

                                <?php if(!empty($v["_child"])): if(is_array($v["_child"])): foreach($v["_child"] as $key=>$vv): ?><label>
                                            <input name="nodeids[]" value="<?php echo ($vv["id"]); ?>" type="checkbox" class="ace" <?php if(in_array(($vv['id']), is_array($data['nodes'])?$data['nodes']:explode(',',$data['nodes']))): ?>checked<?php endif; ?>>
                                            <span class="lbl"> <?php echo ($vv["name"]); ?></span>
                                        </label>

                                        <?php if(!empty($vv["_child"])): if(is_array($vv["_child"])): foreach($vv["_child"] as $key=>$vvv): ?><label>
                                                    <input name="nodeids[]" value="<?php echo ($vvv["id"]); ?>"  type="checkbox" class="ace" <?php if(in_array(($vvv['id']), is_array($data['nodes'])?$data['nodes']:explode(',',$data['nodes']))): ?>checked<?php endif; ?>>
                                                    <span class="lbl"> <?php echo ($vvv["name"]); ?></span>
                                                </label><?php endforeach; endif; endif; endforeach; endif; endif; ?>
                 ...
                 <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input type="radio" class="ace" name="status" value="1" <?php if($data["status"] == 1): ?>checked<?php endif; ?> > <span class="lbl"> 启用 </span>
                        <input type="radio" class="ace" name="status" value="0" <?php if($data["status"] == 0): ?>checked<?php endif; ?> > <span class="lbl"> 禁用 </span>
                    </div>
                </div>
...
<!-- inline scripts related to this page -->
 <script>
    function checknode(obj) {
        $(obj).parent().siblings().find("input").prop('checked',$(obj).is(":checked"));
        /**
         $s =  $(obj).parent().siblings().find("input");
        if (obj.checked) { 
            $s.prop('checked',true);
        } else {
